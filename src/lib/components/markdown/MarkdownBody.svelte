<script lang="ts">
	...
	import { I_TraKeyMarkdownBody, findTranslation } from "./MarkdownBody.translations";
...
<div class="relative m-auto mt-0 h-full max-h-[800px] w-full max-w-[896px] overflow-hidden lg:max-h-full">
	{#if loadingMD}
		<div class="absolute z-10 flex h-full w-full items-center justify-center">
			<span class="z-10">{findTranslation(I_TraKeyMarkdownBody.loading)} README.md...</span>
	...
	{#if !selected}
		<div class=" flex h-full w-full items-center justify-center">
			<div class="flex flex-col items-center justify-center gap-4 p-8">
				...
				<span class="text-2xl">{findTranslation(I_TraKeyMarkdownBody.promtp_title)}</span>
				<span>{findTranslation(I_TraKeyMarkdownBody.prompt_click)} README.md</span>
			</div>
		</div>
	{:else if md}
